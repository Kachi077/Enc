...
about =     '''
                ...
                {}Code name         : {}Kachi
                {}Team              : {} Tech Kachi
                {}E-mail            : {} Onyedikachi817@gmail.com
                {}Github            : {}github.com/Kachi077
                {}Telegram          : {}t.me/Kachi097
                {}Facebook          : {}Upss (Fb Onyedikachi Okoro:v)
                {}Date              : {}16.49 02-02-2021
                
                {}Region            : {}Abia State,Aba, Nigeria'''.format(p,k,p,k,p,k,p,k,p,k,p,k,p,k,p,k,p,k,p,k,p,k,p,k,p,k)
def chat():
        load('Silahkan Tunggu Sebentar---')
	os.system('xdg-open https://api.whatsapp.com/send?phone=+2349035850097&text=Assalamualaikum+Roy+Lu+Masih+Jual+Janda?')
kunci = '''
 	{} ▄▀▀▀▄{} *Hello Dude!!
	{} █   █{} *Where Are You?
	███████         ▄▀▀▄
...
def menu():
	...
	menu()
        ...
        try:
             sp.check_output([
             'am', 'start',
             'https://api.whatsapp.com/send?phone=+2349035850097&text=Report : ' + chat + ''])
        except:
...
def Lima():
        ...
        en = marshal.dumps(c)
        ff = f + 'c'
        open(ff, 'w').write('import marshal Encrypted by KACHI\nexec(marshal.loads(' + repr(en) + '))')
        nm = ('').join(f.split('.')[:1]) + '-enc.py'
        os.rename(ff, nm)
        run('file berhasil di encrypt menjadi %s ' % nm)
def emzb():
...
def mzb():
    ...
    if 'marshal' in b:
        ...
        c.close()
    d = a.replace('.py', '-d.py')
    os.system('python2 enc.py > ' + d)
    e = open(d).read()
    f = open(d, 'w')
    f.write(e + ' \n\n\n\t' + note )
    f.close()
    os.system('rm -rf enc.py')
